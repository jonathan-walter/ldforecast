...
writeRaster(x=psi.pred.raster11, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2011_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster12<-raster(matrix(preds2$psi, nrow(theta12), ncol(theta12), byrow=T), crs=proj4string(theta12))
writeRaster(x=psi.pred.raster12, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2012_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster13<-raster(matrix(preds3$psi, nrow(theta13), ncol(theta13), byrow=T), crs=proj4string(theta13))
writeRaster(x=psi.pred.raster13, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2013_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster14<-raster(matrix(preds4$psi, nrow(theta14), ncol(theta14), byrow=T), crs=proj4string(theta14))
writeRaster(x=psi.pred.raster14, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2014_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster15<-raster(matrix(preds5$psi, nrow(theta15), ncol(theta15), byrow=T), crs=proj4string(theta15))
writeRaster(x=psi.pred.raster15, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2015_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster16<-raster(matrix(preds6$psi, nrow(theta16), ncol(theta16), byrow=T), crs=proj4string(theta16))
writeRaster(x=psi.pred.raster16, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2016_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster17<-raster(matrix(preds7$psi, nrow(theta17), ncol(theta17), byrow=T), crs=proj4string(theta17))
writeRaster(x=psi.pred.raster17, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2017_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster18<-raster(matrix(preds8$psi, nrow(theta18), ncol(theta18), byrow=T), crs=proj4string(theta18))
writeRaster(x=psi.pred.raster18, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2018_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster19<-raster(matrix(preds9$psi, nrow(theta19), ncol(theta19), byrow=T), crs=proj4string(theta19))
writeRaster(x=psi.pred.raster19, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_1_2019_INLA.tif"), 
            format="GTiff", overwrite=TRUE)
...
saveRDS(toSave, here("Data/Model Predictions/threshOpt/accuracy_2001-2010_5km_25km_1_INLA.RDS"))
