...
##theta 2002-2011
theta02<-raster(here("Data/Full Extent/GridTraps2002_full_presence1.tif"))
theta03<-raster(here("Data/Full Extent/GridTraps2003_full_presence1.tif"))
theta04<-raster(here("Data/Full Extent/GridTraps2004_full_presence1.tif"))
theta05<-raster(here("Data/Full Extent/GridTraps2005_full_presence1.tif"))
theta06<-raster(here("Data/Full Extent/GridTraps2006_full_presence1.tif"))
theta07<-raster(here("Data/Full Extent/GridTraps2007_full_presence1.tif"))
theta08<-raster(here("Data/Full Extent/GridTraps2008_full_presence1.tif"))
theta09<-raster(here("Data/Full Extent/GridTraps2009_full_presence1.tif"))
theta10<-raster(here("Data/Full Extent/GridTraps2010_full_presence1.tif"))
theta11<-raster(here("Data/Full Extent/GridTraps2011_full_presence1.tif"))
theta12<-raster(here("Data/Full Extent/GridTraps2012_full_presence1.tif"))
theta13<-raster(here("Data/Full Extent/GridTraps2013_full_presence1.tif"))
theta14<-raster(here("Data/Full Extent/GridTraps2014_full_presence1.tif"))
theta15<-raster(here("Data/Full Extent/GridTraps2015_full_presence1.tif"))
theta16<-raster(here("Data/Full Extent/GridTraps2016_full_presence1.tif"))
theta17<-raster(here("Data/Full Extent/GridTraps2017_full_presence1.tif"))
theta18<-raster(here("Data/Full Extent/GridTraps2018_full_presence1.tif"))
theta19<-raster(here("Data/Full Extent/GridTraps2019_full_presence1.tif"))
...
writeRaster(x=psi.pred.raster11, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2011_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster12<-raster(matrix(preds2$psi, nrow(theta12), ncol(theta12), byrow=T), crs=proj4string(theta12))
writeRaster(x=psi.pred.raster12, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2012_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster13<-raster(matrix(preds3$psi, nrow(theta13), ncol(theta13), byrow=T), crs=proj4string(theta13))
writeRaster(x=psi.pred.raster13, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2013_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster14<-raster(matrix(preds4$psi, nrow(theta14), ncol(theta14), byrow=T), crs=proj4string(theta14))
writeRaster(x=psi.pred.raster14, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2014_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster15<-raster(matrix(preds5$psi, nrow(theta15), ncol(theta15), byrow=T), crs=proj4string(theta15))
writeRaster(x=psi.pred.raster15, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2015_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster16<-raster(matrix(preds6$psi, nrow(theta16), ncol(theta16), byrow=T), crs=proj4string(theta16))
writeRaster(x=psi.pred.raster16, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2016_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster17<-raster(matrix(preds7$psi, nrow(theta17), ncol(theta17), byrow=T), crs=proj4string(theta17))
writeRaster(x=psi.pred.raster17, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2017_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster18<-raster(matrix(preds8$psi, nrow(theta18), ncol(theta18), byrow=T), crs=proj4string(theta18))
writeRaster(x=psi.pred.raster18, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2018_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster19<-raster(matrix(preds9$psi, nrow(theta19), ncol(theta19), byrow=T), crs=proj4string(theta19))
writeRaster(x=psi.pred.raster19, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_1_2019_INLA.tif"), 
            format="GTiff", overwrite=TRUE)
...
saveRDS(toSave, here("Data/Model Predictions/threshOpt/accuracy_2001-2010_5km_50km_1_INLA.RDS"))
