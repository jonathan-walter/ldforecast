...
##connect 2001-2010
connectivity01<-raster(here("JuliaScripts/Full Extent/2001_50km_output/cum_currmap.tif"))
connectivity02<-raster(here("JuliaScripts/Full Extent/2002_50km_output/cum_currmap.tif"))
connectivity03<-raster(here("JuliaScripts/Full Extent/2003_50km_output/cum_currmap.tif"))
connectivity04<-raster(here("JuliaScripts/Full Extent/2004_50km_output/cum_currmap.tif"))
connectivity05<-raster(here("JuliaScripts/Full Extent/2005_50km_output/cum_currmap.tif"))
connectivity06<-raster(here("JuliaScripts/Full Extent/2006_50km_output/cum_currmap.tif"))
connectivity07<-raster(here("JuliaScripts/Full Extent/2007_50km_output/cum_currmap.tif"))
connectivity08<-raster(here("JuliaScripts/Full Extent/2008_50km_output/cum_currmap.tif"))
connectivity09<-raster(here("JuliaScripts/Full Extent/2009_50km_output/cum_currmap.tif"))
connectivity10<-raster(here("JuliaScripts/Full Extent/2010_50km_output/cum_currmap.tif"))
connectivity11<-raster(here("JuliaScripts/Full Extent/2011_50km_output/cum_currmap.tif"))
connectivity12<-raster(here("JuliaScripts/Full Extent/2012_50km_output/cum_currmap.tif"))
connectivity13<-raster(here("JuliaScripts/Full Extent/2013_50km_output/cum_currmap.tif"))
connectivity14<-raster(here("JuliaScripts/Full Extent/2014_50km_output/cum_currmap.tif"))
connectivity15<-raster(here("JuliaScripts/Full Extent/2015_50km_output/cum_currmap.tif"))
connectivity16<-raster(here("JuliaScripts/Full Extent/2016_50km_output/cum_currmap.tif"))
connectivity17<-raster(here("JuliaScripts/Full Extent/2017_50km_output/cum_currmap.tif"))
connectivity18<-raster(here("JuliaScripts/Full Extent/2018_50km_output/cum_currmap.tif"))
connectivity19<-raster(here("JuliaScripts/Full Extent/2019_50km_output/cum_currmap.tif"))
##theta 2002-2011
theta02<-raster(here("Data/Full Extent/GridTraps2002_full_presence10.tif"))
theta03<-raster(here("Data/Full Extent/GridTraps2003_full_presence10.tif"))
theta04<-raster(here("Data/Full Extent/GridTraps2004_full_presence10.tif"))
theta05<-raster(here("Data/Full Extent/GridTraps2005_full_presence10.tif"))
theta06<-raster(here("Data/Full Extent/GridTraps2006_full_presence10.tif"))
theta07<-raster(here("Data/Full Extent/GridTraps2007_full_presence10.tif"))
theta08<-raster(here("Data/Full Extent/GridTraps2008_full_presence10.tif"))
theta09<-raster(here("Data/Full Extent/GridTraps2009_full_presence10.tif"))
theta10<-raster(here("Data/Full Extent/GridTraps2010_full_presence10.tif"))
theta11<-raster(here("Data/Full Extent/GridTraps2011_full_presence10.tif"))
theta12<-raster(here("Data/Full Extent/GridTraps2012_full_presence10.tif"))
theta13<-raster(here("Data/Full Extent/GridTraps2013_full_presence10.tif"))
theta14<-raster(here("Data/Full Extent/GridTraps2014_full_presence10.tif"))
theta15<-raster(here("Data/Full Extent/GridTraps2015_full_presence10.tif"))
theta16<-raster(here("Data/Full Extent/GridTraps2016_full_presence10.tif"))
theta17<-raster(here("Data/Full Extent/GridTraps2017_full_presence10.tif"))
theta18<-raster(here("Data/Full Extent/GridTraps2018_full_presence10.tif"))
theta19<-raster(here("Data/Full Extent/GridTraps2019_full_presence10.tif"))
...
preds1 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds2 <- predictions(result, connect=log10(values(connectivity11)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds3 <- predictions(result, connect=log10(values(connectivity12)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds4 <- predictions(result, connect=log10(values(connectivity13)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds5 <- predictions(result, connect=log10(values(connectivity14)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds6 <- predictions(result, connect=log10(values(connectivity15)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds7 <- predictions(result, connect=log10(values(connectivity16)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds8 <- predictions(result, connect=log10(values(connectivity17)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
preds9 <- predictions(result, connect=log10(values(connectivity18)+1), habitat=values(habitat), psi.crit=perf$thresh.optim)
## true positive rate
...
writeRaster(x=psi.pred.raster11, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2011_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster12<-raster(matrix(preds2$psi, nrow(theta12), ncol(theta12), byrow=T), crs=proj4string(theta12))
writeRaster(x=psi.pred.raster12, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2012_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster13<-raster(matrix(preds3$psi, nrow(theta13), ncol(theta13), byrow=T), crs=proj4string(theta13))
writeRaster(x=psi.pred.raster13, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2013_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster14<-raster(matrix(preds4$psi, nrow(theta14), ncol(theta14), byrow=T), crs=proj4string(theta14))
writeRaster(x=psi.pred.raster14, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2014_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster15<-raster(matrix(preds5$psi, nrow(theta15), ncol(theta15), byrow=T), crs=proj4string(theta15))
writeRaster(x=psi.pred.raster15, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2015_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster16<-raster(matrix(preds6$psi, nrow(theta16), ncol(theta16), byrow=T), crs=proj4string(theta16))
writeRaster(x=psi.pred.raster16, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2016_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster17<-raster(matrix(preds7$psi, nrow(theta17), ncol(theta17), byrow=T), crs=proj4string(theta17))
writeRaster(x=psi.pred.raster17, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2017_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster18<-raster(matrix(preds8$psi, nrow(theta18), ncol(theta18), byrow=T), crs=proj4string(theta18))
writeRaster(x=psi.pred.raster18, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2018_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster19<-raster(matrix(preds9$psi, nrow(theta19), ncol(theta19), byrow=T), crs=proj4string(theta19))
writeRaster(x=psi.pred.raster19, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_50km_10_2019_INLA.tif"), 
            format="GTiff", overwrite=TRUE)
...
saveRDS(toSave, here("Data/Model Predictions/threshOpt/accuracy_2001-2010_5km_50km_10_INLA.RDS"))
