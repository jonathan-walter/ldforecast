...
##connect 2001-2010
connectivity01<-raster(here("JuliaScripts/Full Extent/2001_25km_output/cum_currmap.tif"))
connectivity02<-raster(here("JuliaScripts/Full Extent/2002_25km_output/cum_currmap.tif"))
connectivity03<-raster(here("JuliaScripts/Full Extent/2003_25km_output/cum_currmap.tif"))
connectivity04<-raster(here("JuliaScripts/Full Extent/2004_25km_output/cum_currmap.tif"))
connectivity05<-raster(here("JuliaScripts/Full Extent/2005_25km_output/cum_currmap.tif"))
connectivity06<-raster(here("JuliaScripts/Full Extent/2006_25km_output/cum_currmap.tif"))
connectivity07<-raster(here("JuliaScripts/Full Extent/2007_25km_output/cum_currmap.tif"))
connectivity08<-raster(here("JuliaScripts/Full Extent/2008_25km_output/cum_currmap.tif"))
connectivity09<-raster(here("JuliaScripts/Full Extent/2009_25km_output/cum_currmap.tif"))
connectivity10<-raster(here("JuliaScripts/Full Extent/2010_25km_output/cum_currmap.tif"))
connectivity11<-raster(here("JuliaScripts/Full Extent/2011_25km_output/cum_currmap.tif"))
connectivity12<-raster(here("JuliaScripts/Full Extent/2012_25km_output/cum_currmap.tif"))
connectivity13<-raster(here("JuliaScripts/Full Extent/2013_25km_output/cum_currmap.tif"))
connectivity14<-raster(here("JuliaScripts/Full Extent/2014_25km_output/cum_currmap.tif"))
connectivity15<-raster(here("JuliaScripts/Full Extent/2015_25km_output/cum_currmap.tif"))
connectivity16<-raster(here("JuliaScripts/Full Extent/2016_25km_output/cum_currmap.tif"))
connectivity17<-raster(here("JuliaScripts/Full Extent/2017_25km_output/cum_currmap.tif"))
connectivity18<-raster(here("JuliaScripts/Full Extent/2018_25km_output/cum_currmap.tif"))
connectivity19<-raster(here("JuliaScripts/Full Extent/2019_25km_output/cum_currmap.tif"))
##theta 2002-2011
...
writeRaster(x=psi.pred.raster11, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2011_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster12<-raster(matrix(preds2$psi, nrow(theta12), ncol(theta12), byrow=T), crs=proj4string(theta12))
writeRaster(x=psi.pred.raster12, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2012_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster13<-raster(matrix(preds3$psi, nrow(theta13), ncol(theta13), byrow=T), crs=proj4string(theta13))
writeRaster(x=psi.pred.raster13, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2013_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster14<-raster(matrix(preds4$psi, nrow(theta14), ncol(theta14), byrow=T), crs=proj4string(theta14))
writeRaster(x=psi.pred.raster14, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2014_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster15<-raster(matrix(preds5$psi, nrow(theta15), ncol(theta15), byrow=T), crs=proj4string(theta15))
writeRaster(x=psi.pred.raster15, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2015_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster16<-raster(matrix(preds6$psi, nrow(theta16), ncol(theta16), byrow=T), crs=proj4string(theta16))
writeRaster(x=psi.pred.raster16, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2016_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster17<-raster(matrix(preds7$psi, nrow(theta17), ncol(theta17), byrow=T), crs=proj4string(theta17))
writeRaster(x=psi.pred.raster17, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2017_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster18<-raster(matrix(preds8$psi, nrow(theta18), ncol(theta18), byrow=T), crs=proj4string(theta18))
writeRaster(x=psi.pred.raster18, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2018_INLA.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster19<-raster(matrix(preds9$psi, nrow(theta19), ncol(theta19), byrow=T), crs=proj4string(theta19))
writeRaster(x=psi.pred.raster19, filename=here("./Data/Model Predictions/threshOpt/2001_2010_5km_25km_10_2019_INLA.tif"), 
            format="GTiff", overwrite=TRUE)
...
saveRDS(toSave, here("Data/Model Predictions/threshOpt/accuracy_2001-2010_5km_25km_10_INLA.RDS"))
