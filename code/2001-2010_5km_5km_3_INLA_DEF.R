...
preds1 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[10,2], 132000))
## true positive rate
...
preds2 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[11,2], 132000))
## true positive rate
...
preds3 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[12,2], 132000))
## true positive rate
...
preds4 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[13,2], 132000))
## true positive rate
...
preds5 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[14,2], 132000))
## true positive rate
...
preds6 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[15,2], 132000))
## true positive rate
...
preds7 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[16,2], 132000))
## true positive rate
...
preds8 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[17,2], 132000))
## true positive rate
...
preds9 <- predictions(result, connect=log10(values(connectivity10)+1), habitat=values(habitat),
                      defol=rep(defoliation[18,2], 132000))
## true positive rate
...
attach(mtcars)
par(mfrow=c(1,3))
plot(year, accuracy, main="5km Radius, Presence 3 w defol", 
     xlab="Year ", ylab="Total Accuracy", pch=19)
plot(year, accuracypos, main="5km Radius, Presence 3 w defol", 
     xlab="Year ", ylab="True Positive", pch=19)
plot(year, falsepos, main="5km Radius, Presence 3 w defol", 
...
writeRaster(x=psi.pred.raster11, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2011_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster12<-raster(matrix(preds2$psi, nrow(theta12), ncol(theta12), byrow=T), crs=proj4string(theta12))
writeRaster(x=psi.pred.raster12, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2012_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster13<-raster(matrix(preds3$psi, nrow(theta13), ncol(theta13), byrow=T), crs=proj4string(theta13))
writeRaster(x=psi.pred.raster13, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2013_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster14<-raster(matrix(preds4$psi, nrow(theta14), ncol(theta14), byrow=T), crs=proj4string(theta14))
writeRaster(x=psi.pred.raster14, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2014_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster15<-raster(matrix(preds5$psi, nrow(theta15), ncol(theta15), byrow=T), crs=proj4string(theta15))
writeRaster(x=psi.pred.raster15, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2015_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster16<-raster(matrix(preds6$psi, nrow(theta16), ncol(theta16), byrow=T), crs=proj4string(theta16))
writeRaster(x=psi.pred.raster16, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2016_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster17<-raster(matrix(preds7$psi, nrow(theta17), ncol(theta17), byrow=T), crs=proj4string(theta17))
writeRaster(x=psi.pred.raster17, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2017_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster18<-raster(matrix(preds8$psi, nrow(theta18), ncol(theta18), byrow=T), crs=proj4string(theta18))
writeRaster(x=psi.pred.raster18, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2018_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

psi.pred.raster19<-raster(matrix(preds9$psi, nrow(theta19), ncol(theta19), byrow=T), crs=proj4string(theta19))
writeRaster(x=psi.pred.raster19, filename=here("./Data/Reduced/2001_2010_5km_5km_3_2019_INLA_DEF.tif"), 
            format="GTiff", overwrite=TRUE)

## Plots!!
par(mfrow = c(3, 2))
plot(psi.pred.raster11, main="5km, 3, predictions 2011 defol")
plot(theta11, main="5km, 3, actual 2011")

plot(psi.pred.raster12, main="5km, 3, predictions 2012 defol")
plot(theta12, main="5km, 3, actual 2012")

plot(psi.pred.raster13, main="5km, 3, predictions 2013 defol")
plot(theta13, main="5km, 3, actual 2013")

plot(psi.pred.raster14, main="5km, 3, predictions 2014 defol")
plot(theta14, main="5km, 3, actual 2014")

plot(psi.pred.raster15, main="5km, 3, predictions 2015 defol")
plot(theta15, main="5km, 3, actual 2015")

plot(psi.pred.raster16, main="5km, 3, predictions 2016 defol")
plot(theta16, main="5km, 3, actual 2016")

plot(psi.pred.raster17, main="5km, 3, predictions 2017 defol")
plot(theta17, main="5km, 3, actual 2017")

plot(psi.pred.raster18, main="5km, 3, predictions 2018 defol")
plot(theta18, main="5km, 3, actual 2018")

plot(psi.pred.raster19, main="5km, 3, predictions 2019 defol")
...
saveRDS(toSave, here("Data/Model Predictions/2001-2010_5km_5km_3_INLA_DEF.RDS"))
